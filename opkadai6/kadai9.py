...
with open("inv_seireishi.txt", encoding="utf-8") as f:
    for line in f:
        index=line.split()
        inv[index[0]]={}
        word=index[1].split(",")
...
with open("doc_id_city.txt", encoding="utf-8") as f2:
    for line in f2:
        line_cut=line.split()
        doc.append(Document(line_cut[0],line_cut[1],0.0))


with open("doc_datac.txt", encoding="utf-8") as f3:
    for line in f3:
        line_data=line.split()
        doc[int(line_data[0])].vec_length=line_data[2]

doc_cos= [[0 for i in range(n_docs)] for i in range(n_docs)]#cos用
...
for a in range(n_docs):
    for b in range(n_docs):
        ...
        # if (a==0 and b==0):
        #     print(doc_cos[a][b],dot_product,length_a*length_b)



with open("doc_simc.txt", "w", encoding="utf-8") as f4:
